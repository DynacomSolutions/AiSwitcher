 ...
 * 1. Host header must be loopback-shaped (DNS-rebinding guard: a hostile
 *    web page cannot make the browser resolve its own domain to 127.0.0.1
 *    and keep a valid-looking Host).
 * 2. Every /api request must present EITHER the per-boot bearer token OR
 *    originate from a loopback peer address.
...
export function allowedHost(hostHeader: string | undefined, port: number): boolean {
  if (!hostHeader) return false;
  const bare = hostHeader.toLowerCase().split(":")[0];
  if (bare !== "127.0.0.1" && bare !== "localhost" && bare !== "::1") return false;
...
export async function consoleGuard(deps: GuardDeps, c: Context, next: Next): Promise<Response | void> {
  if (!allowedHost(c.req.header("Host"), deps.port)) {
  ...
  const localOk = bearerOk || isLoopbackAddress(typeof remoteAddress === "string" ? remoteAddress : undefined);
  if (!localOk) {
